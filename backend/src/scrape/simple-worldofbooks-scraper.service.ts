...
@Injectable()
export class SimpleWorldOfBooksScraperService {
  private readonly logger = new Logger(SimpleWorldOfBooksScraperService.name);

  constructor(private prisma: PrismaService) {}

  async scrapeWorldOfBooks(): Promise<{ message: string; stats: any }> {
    this.logger.log('Starting simple World of Books scraping...');
    ...
    try {
      ...
      // 3. Create comprehensive book collection for each category
      const bookCategories: Record<string, Array<{ title: string; author: string; price: number; isbn: string; publisher: string }>> = {
        'fiction': [
      ...
      for (const category of allCategories) {
        ...
        for (const book of booksForCategory) {
          try {
            ...
            this.logger.error(`Failed to save book ${book.title} in ${category.slug}:`, error);
          }
        }
      }
      ...
      // 4. Create product details and reviews for all products
      ...
      for (const product of allProducts) {
        try {
          ...
          this.logger.error(`Failed to create details for ${product.title}:`, error);
        }
      }

      this.logger.log('Simple World of Books scraping completed', stats);
      return { message: 'Scraping completed successfully', stats };
    } catch (error) {
      this.logger.error('Simple World of Books scraping failed:', error);
      stats.errors++;
      throw error;
    }
  }
